...
describe('dashboard v3 evolution workspace contract', () => {
  ...
  it('shows the required project-level evolution status groups', () => {
    ...
    expect(componentSource).toContain('recommendedActions')
    expect(componentSource).toContain('rollback')
    expect(componentSource).toContain('freeze')
    expect(componentSource).toContain('preview')
    expect(componentSource).toContain('backup')
  })
})
