...
- 2026-04-23 更新：
  - `frontend-v3/` 这条 React 重写链已退出当前默认运行面
  - 当前 `/v3` 不是独立 React 前端，而是 **严格复用 v1 dashboard shell 的兼容入口**
  - 运行时只保留 `/v3/skills`、`/v3/project`、`/v3/config`
  - `/v3/assets/*` 与 `build:dashboard-v3` 已退役，避免继续暴露任何 v1 不存在的额外表面
  - 因此本文件以下关于 `frontend-v3` 的内容仅保留为一次失败重写的迁移记录，不再代表当前生产基线
