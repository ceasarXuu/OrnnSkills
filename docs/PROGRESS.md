...
- ✅ 放宽 `ornn start` 空注册表启动限制：没有任何已初始化项目时，daemon 与 dashboard 仍会正常拉起，后续项目执行 `ornn init` 后再被全局 daemon 接管。
- 📝 记录启动经验：全局 daemon/dashboard 是产品入口，不应被“当前已经注册了项目”这个业务状态阻断；空项目列表应作为 dashboard 的可用空态处理，而不是 CLI 启动错误。
