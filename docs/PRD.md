...
V2.0 的主界面采用 3 个一级页面，不再把“演进”“详情”“库”拆成独立一级入口，而是统一收口到围绕 skill 管理的主工作流中:

### 10.1 一级页面

1. `主页`
   - 面向全局 skill 资产的总览
   - 总 skill 家族数、安装实例数、活跃数、沉睡数
   - 待处理提醒，例如长期未使用、近期高频但负反馈偏高
   - 关键入口，例如进入 Skills 页处理某个技能
2. `Skills`
   - V2.0 的核心工作台
   - 承担 skill 管理、使用判断、实例分布查看、修订历史和演进入口
   - 支持 `Skill 视角` 与 `项目视角` 两种工作方式
3. `配置`
   - 平台级配置能力
...
`Skills` 页不是单一列表，而是一个围绕 skill 的双视角工作区:
...
- 项目侧边导航只出现在 `Skills -> 项目工作台`
- `主页` 和 `配置` 不带项目侧边导航
- `技能库` 的主列表单位是 `Skill Family`，不是实例，也不是 shadow
- `项目工作台` 的主列表单位是“项目内的 skill family 组”，组内再展示宿主实例
