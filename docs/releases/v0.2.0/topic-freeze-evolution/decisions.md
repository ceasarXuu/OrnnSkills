...
| D5 | 影子 skills 机制纳入冻结 | 影子 skills 本身是演化能力的一部分：冻结期间不物化（新项目不扫描/不复制/不建索引）、不更新、dashboard 技能库展示也隐藏；已存在的影子数据冻结期间不展示；解冻后自动补物化 | 不得在冻结期间物化或展示影子 skills；不得把 bootstrap 物化视为"索引同步"豁免 | 用户明确"影子skills 也是为了做演化能力才加入的，本身也是演化能力的一部分，需要纳入冻结"；此决策覆盖此前对抗性审查 F2 的"索引同步豁免"结论 | 冻结期间仍产生影子内容或技能库仍展示影子数据 | user-confirmed-direct: 2026-08-09 用户确认"需要纳入冻结"，并确认展示也隐藏、不物化 | active |
