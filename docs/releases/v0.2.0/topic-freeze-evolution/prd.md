...
- **冻结影子 skills 机制（D5）**：新项目不物化影子（不扫描/不复制/不建索引/不创建初始版本）；已存在影子数据冻结期间不展示
- 隐藏演化相关 UI 入口与技能库展示（D2/D5）
- 冻结开关机制（配置驱动，可逆）
...
- 不删除演化代码（临时冻结，保留恢复能力）
- 不重设计演化闭环（另行规划）
- 不冻结 trace 采集、项目工作台、配置编辑、市场等非影子能力
...
- 冻结期间影子 skills 整体停摆（D5）：不物化、不更新、技能库不展示；解冻后重启 daemon 自动补物化
