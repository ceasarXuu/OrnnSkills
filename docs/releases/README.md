...
- topic 命名：`topic-<kebab-case 主题>`，如 `topic-evolution-write-side`、`topic-ci-fix`。一个 feature 或一个 bug 修复对应一个 topic；一个版本内有多个工作单元时，按单元数量建立多个 topic 子文件夹。
...
- 需求尚未确定的版本（如当前 v0.2.0）只保留空目录（`.gitkeep`），待需求确定后再按 topic 结构补齐。
...
| 版本 | 状态 | 说明 |
|---|---|---|
| [v0.1.13](v0.1.13/prd.md) | released（已发布 2026-05-01） | 当前已发布版本锁定，版本级三件套 |
| [v0.2.0](v0.2.0) | draft | 需求未定，暂为空的 topic 工作区 |
