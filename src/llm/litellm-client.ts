...
import {
  buildEmptyResponseErrorMessage,
  extractCompletionContent,
  getNextStructuredResponseMaxTokens,
  getStructuredResponseAttempts,
  type CompletionExtractionDiagnostics,
  type CompletionExtractionResult,
} from './litellm-client-extract.js';

export type { LiteLLMCompletionOptions } from './litellm-types.js';
...
export class LiteLLMClient implements LLMInstance {
  ...
  private extractContent(
    ...
    return extractCompletionContent(response, responseFormat);
  }

  private getStructuredResponseAttempts(
    responseFormat: LiteLLMCompletionOptions['responseFormat']
  ): number {
    return getStructuredResponseAttempts(this.provider, responseFormat);
  }
  ...
  private getNextStructuredResponseMaxTokens(
    ...
    return getNextStructuredResponseMaxTokens(
      this.provider,
      this.maxTokens,
      currentMaxTokens,
      responseFormat,
  ...
  private buildEmptyResponseErrorMessage(
    ...
    return buildEmptyResponseErrorMessage(
      this.provider,
      this.modelName,
      responseFormat,
      attempts,
